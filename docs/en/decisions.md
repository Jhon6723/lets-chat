...
| Prekey bundle fetch gated to contact edges | Accepted | 2026-09-21 | An open directory (Signal model) leaks device enumeration and lets anyone drain a victim's one-time-prekey pool; gating kills both. Bootstrap stays possible: a pending contact request grants the *target* one-directional fetch of the requester's bundles. Revises ADR-0006's "public-key directory" wording |
